...
public class RoomMeasurementTestTopologyDriver {
    ...
    private Serde<MeasurementAnomaly> outputEventSerde = new JsonSerde<>(MeasurementAnomaly.class);
    private Serde<TemperatureMeasurementMessage> inputMessageSerde = new JsonSerde<>(TemperatureMeasurementMessage.class);
    ...
    public RoomMeasurementTestTopologyDriver(AnomalyDetector anomalyDetector) {
        configureDeserializer(outputEventSerde.deserializer(), String.class, MeasurementAnomaly.class, false);
        configureDeserializer(keySerde.deserializer(), String.class, null, true);

        final StreamsBuilder builder = new StreamsBuilder();

        KStream<String, TemperatureMeasurementMessage> input = builder.stream(INPUT_TOPIC, Consumed.with(keySerde, inputMessageSerde));
        AnomalyDetectorProcess anomalyDetectorProcess = new AnomalyDetectorProcess(new AnomalyDetectorService(anomalyDetector));

        KStream<String, MeasurementAnomaly> output = anomalyDetectorProcess.process().apply(input);
        output.to(OUTPUT_TOPIC);

        Topology topology = builder.build();
        testDriver = new TopologyTestDriver(topology, getStreamsConfiguration());
        inputTopic = testDriver.createInputTopic(INPUT_TOPIC, keySerde.serializer(), inputMessageSerde.serializer());
        outputTopic = testDriver.createOutputTopic(OUTPUT_TOPIC, keySerde.deserializer(), outputEventSerde.deserializer());
    }
    ...
    public List<MeasurementAnomaly> loadOutputMessages(int expectedCount) {
        List<MeasurementAnomaly> outputsEvents = new LinkedList<>();
        int receivedCount = 0;
        while (receivedCount < expectedCount) {
            ...
            receivedCount++;
            outputsEvents.add(record);
        }
        return outputsEvents;
    }
